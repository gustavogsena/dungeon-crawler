...
const Textfield = ({ id, label, labelStyle, type, funcaoOnChange, value, step, min, classStyle, placeholder = '', required = true, disabled = false }: TextfieldType) => {
    ...
    return (
        <div className="flex flex-col w-full">
            ...
            <input
                className={`bg-amarelo-200 shadow-sm w-full rounded-[10px] text-base p-3 box-border text-verde-600 placeholder-verde-600 placeholder-opacity-100 ${classStyle}`}
                id={id}
                type={type}
                required={required}
